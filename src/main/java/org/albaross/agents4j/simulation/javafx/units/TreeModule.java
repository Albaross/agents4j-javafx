...
public class TreeModule<T> extends HBox implements ViewModule {
	...
	public TreeModule(String title) {
		...
		this.tree.addEventHandler(MouseEvent.MOUSE_CLICKED, (event) -> {
			...
			T value = ((TreeItem<T>) ((TreeView<T>) event.getSource()).getSelectionModel().getSelectedItem()).getValue();
	...
	public TreeModule<T> addListener(ItemDoubleClickHandler<T> listener) {
